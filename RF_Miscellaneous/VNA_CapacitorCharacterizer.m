...
fnom=200e6; % <--------------Enter the frequency at which you designed your capacitor
f = datos(:,1);
re = datos(:,2);
im = datos(:,3);
...
[~, indxC] = min(abs(datos(:, 1) - fnom));
Cnom=Xfreq(indxC); % Obtains measured capacitance
...
syms L;
syms Rnom;
Ls= double(vpasolve(1/sqrt(Cnom*L)==2*pi*f_srf,L));
w_srf = 2*pi*f_srf;
Zmod = Rnom + 1i*w_srf*Ls-1i/(w_srf*Cnom);
[~, indxR] = min(abs(datos(:, 1) - f_srf));
Rmod=real(double(vpasolve((Zmod-Z0)/(Zmod+Z0)==Gamma(indxR),Rnom)));

Zcal = Rmod + 1i * (2*pi.*f * Ls - 1./(2*pi.*f * Cnom));
Xfreqcal=-1./(2*pi*f.*imag(Zcal));
%---Resistance
subplot(3,1,1);
...
title('Capacitancia (C) [F]');
ylabel('F');
legend('C medido', 'C modelo');
grid on;
disp('Resultados del Modelo [Rs,Ls,C]:');
fprintf('Rs: %.3e Ohm, Ls: %.3e F, C: %.3e H\n', Rmod, Ls, Cnom);